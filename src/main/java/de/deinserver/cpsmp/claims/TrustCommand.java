...
public final class TrustCommand implements CommandExecutor {
    ...
    public TrustCommand(CPSMPPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (!player.hasPermission(ClaimPermission.CLAIM_TRUST)) {
            plugin.getMessageManager().sendPrefixed(player, "claim.no-permission");
            return true;
        }
        if (args.length < 1) {
            plugin.getMessageManager().sendPrefixed(player, "claim.trust-usage");
            return true;
        }
        ...
        claims.trustPlayer(player, target);
        return true;
    }
}
