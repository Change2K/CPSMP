...
- **V4.0.1 claim outline**: `claims.visuals.mode` defaults to **`display`** (BlockDisplay border: visual-only, non-blocking, visible even when the client reduces particles). **`particles`** is the alternate mode and the usual **fallback** if display spawn fails. Per-player `Player#setWorldBorder` is **opt-in only** (`allow-worldborder-mode` + `mode: worldborder_if_safe`) because the API applies a real border that can **block movement**; not recommended for routine outlines.
- **V4.0.2**: default **`display`** mode uses **BlockDisplay** entities along claim edges (plus **`particles`** when configured or as fallback). **Merge** (`/merge all`, GUI) unions trust and keeps the lowest visible claim number on the surviving `claim_id`. Admin **`/claim <Spieler> <Claim>`** teleports with chunk load + safe-surface scan (`cpsmp.claim.admin` or `cpsmp.claim.teleport.admin`).
- **V4.0.3 anti-encasement**: CPSMP-native; **no** WorldGuard/GriefPrevention dependency. **Direct edge** band + bounded **access-integrity** flood-fill (in-memory cache only, no per-event SQLite). Does **not** restrict owners/trusted inside claims. **`/claimexit`** uses existing `TeleportService`. Default new claim size **24×24** via `default-size-x/z`; legacy `default-radius-x/z` supported.
- **Visible claim numbers**: players see **per-owner** indices (`#1`, `#2`, …). The SQLite **`claim_id`** remains the internal primary key; trust rows still reference `claim_id`. Admin delete is **`/claimadmin delete <player> <number>`**; **`/claimadmin deleteglobal <id>`** is for emergencies only.
- **V4.1 claim flags**: CPSMP-native per-claim booleans in SQLite table **`claim_flags`** (in-memory cache; no SQLite on protection events). Global `claims.protection.*` remains the master switch; flags refine behavior per claim. **No biome flags** in V4.1 (reserved for a future release). Commands: **`/claimflags`**, **`/claim flags`**, **`/plot flags`**, GUI Details → Flags. Permissions: `cpsmp.claim.flags`, `cpsmp.claim.flags.admin`. No external claim-plugin hooks.
